...
def start_file_log(path: str, append: bool = False) -> bool:
    ...
    try:
        ...
        with open(path, mode, encoding="utf-8") as f:
            f.write("\n" if append else "")
            f.write("=" * 80 + "\n")
            f.write(time.strftime("%Y-%m-%d %H:%M:%S") + "  DubFlow / AutoDubVN process log\n")
            f.write("=" * 80 + "\n")
        with _LOG_LOCK:
            _LOG_FILE = path
...
def run(cmd: List[str], check: bool = True, quiet: bool = True,
            ...
            while not reader_finished:
                if owner_cancel_event is not None and owner_cancel_event.is_set():
                    try:
                        ...
                        try:
                            ...
                            pass
                    raise InterruptedError("The running command was cancelled")
                if timeout is not None and time.monotonic() - started > timeout:
                    try:
                        ...
                        pass
                    raise RuntimeError(
                        f"The command ran longer than {timeout:.0f}s and was stopped: "
                        f"{' '.join(cmd[:4])}...")
                try:
            ...
            while True:
                ...
                if timeout is not None and time.monotonic() - started > timeout:
                    try:
                        ...
                        pass
                    raise RuntimeError(f"The command ran longer than {timeout:.0f}s and was stopped: {' '.join(cmd[:4])}...")
                try:
    ...
    if check and res.returncode != 0:
        ...
        raise RuntimeError(f"Command failed ({res.returncode}): {' '.join(cmd[:4])}...\n{err}")
    return res
...
def ffmpeg_dir_to_path(fdir: str) -> bool:
    ...
    if not os.path.isdir(raw):
        log(f"ffmpeg_dir does not exist: {raw!r} - ignoring it and using the system PATH.", "warn")
        return False
    ...
    if not os.path.exists(ffmpeg) or not os.path.exists(ffprobe):
        log(f"ffmpeg_dir is missing ffmpeg/ffprobe: {raw!r} - ignoring it and using the system PATH.", "warn")
        return False
    if not _tool_runs(ffmpeg) or not _tool_runs(ffprobe):
        log(f"ffmpeg_dir has the files but they cannot run (possibly blocked by Windows): {raw!r} - ignoring it and using the system PATH.", "warn")
        return False
...
def require(name: str, hint: str = "") -> None:
    path = which(name)
    if not path:
        log(f"Missing tool '{name}'. {hint}", "err")
        raise SystemExit(1)
...
def has_nvenc() -> bool:
    ...
    try:
        ...
        _NVENC_OK = False
    if not _NVENC_OK:
        log("NVENC test: cannot encode (old driver or busy GPU). "
            "Using the stable CPU x264 encoder instead.", "warn")
    return _NVENC_OK
