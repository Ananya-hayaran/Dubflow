...
class Placement:
    ...
    # DubFlow: clip vẫn dài hơn cửa sổ của nó dù đã tăng tới tốc độ tối đa.
    # Không cắt, không đẩy câu sau - chỉ đánh dấu để người dùng xem lại.
...
def fit_segments_strict(
    ...
    `tolerance` (DubFlow, tỉ lệ tương đối, mặc định 0 = hành vi cũ): clip dài hơn
    cửa sổ không quá `tolerance` (vd 0.05 = 5%) được coi là vừa, giữ nguyên tốc
    độ. Clip vẫn dài hơn cửa sổ sau khi đã tăng tới `max_speed` được gắn
    `needs_review=True` (kể cả khi `trim_overflow` cắt nó).
    """
    n = len(starts)
    if n != len(natural_durations):
        raise ValueError("starts và natural_durations phải cùng độ dài")
    ...
    for i in range(n):
        ...
        if ends is not None:
            ...
            slot = max(0.01, next_start - start - min_gap)
        speed = base_speed
        if nat > slot * (1.0 + tolerance) and slot > 1e-3:
            speed = min(max_speed, max(base_speed, nat / slot))
        elif slot <= 1e-3:
            speed = max_speed

        final_dur = nat / speed if speed > 0 else nat
        limit = slot * (1.0 + tolerance)
        # Still longer than its window after the maximum allowed speed-up.
        needs_review = final_dur > limit + 1e-6
        trimmed = False
        ...
        if trim_overflow and final_dur > limit + 1e-6:
            final_dur = slot
            trimmed = True
...
def summarize(placements: List[Placement]) -> dict:
    ...
    return {
        ...
        "needs_review": sum(1 for p in placements if getattr(p, "needs_review", False)),
