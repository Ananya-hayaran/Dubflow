...
def _parse_progress_line(line: str) -> Optional[Dict]:
    ...
    stream = f" · stream {format_id}" if format_id and format_id != "NA" else ""
    if str(status).lower() == "finished":
        text = f"Download {ident}{stream}: 100% · received, merging/checking"
    else:
        pieces = [f"Download {ident}{stream}"]
        ...
        if eta is not None:
            pieces.append("ETA " + _human_eta(eta))
        text = ": ".join(pieces[:2])
        if len(pieces) > 2:
            text += " · " + " · ".join(pieces[2:])
    return {
...
def _download_speed_options(concurrent_fragments=8,
    ...
    notes = [f"{n_frag} parallel fragments"]
    ...
    if choice_l == "auto":
        return cmd, ", ".join(notes + ["yt-dlp built-in downloader"])
    ...
    if downloader:
        ...
        if "aria2c" in base:
            ...
            notes.append(f"aria2c {conn} connections")
        else:
            notes.append(f"external downloader: {choice}")
    return cmd, ", ".join(notes)
...
def _validate_download_file(path: str, cookie_hint: str,
    ...
    if not path or not os.path.isfile(path):
        raise RuntimeError("The downloaded file was not found.")

    # Kiểm tra lại file THỰC SỰ có hình, không chỉ tin bộ tải báo thành công.
    w, h = ffprobe_video_size(path)
    if w == 0 or h == 0:
        raise RuntimeError(
            f"The downloaded file has AUDIO ONLY, no video ({os.path.basename(path)}). "
            "Common cause: the site only serves the requested resolution when you are "
            "LOGGED IN. Fix: {hint} Then retry (delete this broken file in the "
            "downloads folder before running again). If it still fails, "
            "lower download.quality to 480 or update yt-dlp: "
            "python -m pip install -U yt-dlp".format(hint=cookie_hint)
        )
    if not ffprobe_has_stream(path, "a"):
        raise RuntimeError(
            f"The downloaded file has VIDEO ONLY, no audio ({os.path.basename(path)}). "
            f"{cookie_hint} Then delete the broken file in the downloads folder and download again."
        )

    log("Checking that the video is not a black placeholder...", "info")
    if ffprobe_is_blank_video(path):
        raise RuntimeError(
            f"The downloaded file has audio but the picture is ENTIRELY BLACK ({os.path.basename(path)}). "
            "Some sites return a fake placeholder picture when the requested resolution is locked behind "
            f"login/VIP, even though the stream reports the right size. {cookie_hint} Then "
            "delete the broken file in the downloads folder and download again. If it is still black even with "
            "cookies, lower download.quality to 480."
        )

    log(f"Downloaded: {os.path.basename(path)}", "ok")
    if progress_callback:
        progress_callback({"status": "complete", "percent": 100.0,
                           "text": "Download complete: " + os.path.basename(path),
                           "path": path})
    return path
...
def download_video(
    ...
    format_selector: chuỗi -f của yt-dlp; nếu đặt thì thay bảng `quality`
    (DubFlow dùng để ưu tiên H.264 cho phép copy luồng hình không mã hoá lại).
    """
    url = extract_url(url)
    if not url:
        raise RuntimeError("No valid URL found. Paste a link that starts with http:// or https://.")
    os.makedirs(out_dir, exist_ok=True)
    cookies_from_browser = _normalise_cookie_browser(cookies_from_browser)
    _cookie_hint = (
        "Check the link. For age-restricted or login-only videos, set "
        "download.cookies_from_browser: chrome (or edge/firefox - a browser you are "
        "ALREADY logged in to) in the config file."
    )
    ...
    if bilibili_direct.is_bilibili_url(url):
        log("Analysing the video with the direct Bilibili downloader...", "step")
        ...
        def _on_direct_progress(info: Dict) -> None:
            ...
            text = str(info.get("text") or "Downloading from Bilibili...")
            log(text, "info")
            if progress_callback:
        ...
        try:
            ...
            log("Direct Bilibili: %s · %s stream · fastest CDN selected automatically."
            ...
            log("The direct Bilibili downloader could not fetch the video (%s); "
                "switching to yt-dlp..." % str(direct_error)[:220], "warn")

    fmt = str(format_selector).strip() if format_selector else \
        _QUALITY.get(str(quality), _QUALITY["best"])
    ...
    log("Downloading video (yt-dlp)...", "step")
    log(f"Download acceleration: {speed_note}.", "info")
    ...
    def _on_download_heartbeat(elapsed: float) -> None:
        text = ("yt-dlp is still running · no new data lines for 15 seconds "
                f"· total time {_human_eta(elapsed)}")
        log(text, "dim")
        if progress_callback:
    ...
    try:
        ...
        if _browser_cookie_failed(e) and "--cookies-from-browser" in cmd:
            ...
            log(f"The {_cookie_browser_label(cookies_from_browser)} cookie database is locked "
                "(fully close the browser if you need the logged-in version); "
                "retrying as a public video without cookies...",
                "warn")
    ...
    if res is None:
        e = first_error or RuntimeError("yt-dlp returned no result")
        ...
        if _external_downloader_failed(e) and "--downloader" in cmd:
            log("aria2c could not download this URL; switching to yt-dlp and continuing...",
                "warn")
        ...
        if res is None and _transient_download_failed(first_error or e):
            ...
            retry_qualities = [(fmt, "current quality")]
            if str(quality).strip().lower() == "best":
                retry_qualities.append((_QUALITY["480"], "480p fallback"))
            ...
            for retry_fmt, quality_note in retry_qualities:
                for attempt in range(attempts_per_quality):
                    ...
                    retry_cmd = _build_download_command(
                        ...
                        cookies_file=cookies_file)
                    if use_resume:
                        log(f"The CDN dropped the connection; resuming the .part file "
                            f"({quality_note}, attempt {attempt + 1}/{attempts_per_quality}, "
                            "1 fragment, 10 MiB chunks)...", "warn")
                    else:
                        log(f"CDN/Range error; restarting cleanly ({quality_note}, "
                            "1 fragment, discarding the old .part file)...", "warn")
                    try:
        ...
        if res is None:
            ...
            raise RuntimeError(
                f"Download failed. {_cookie_hint}\nDetails: {final_error}") from final_error
