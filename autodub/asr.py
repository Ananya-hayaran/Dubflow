...
def speech_map_from_audio(audio_path: str, duration: float = 0.0,
    ...
    try:
        ...
        log(f"Could not detect speech regions to build the speech map: {e}", "warn")
        return None
...
def ensure_speech_map(out_dir: str, stem: str,
    ...
    if m is not None:
        ...
        log(f"Reusing the saved speech map ({len(m)} marks): {os.path.basename(path)}",
            "ok")
        return m
    ...
    if audio is None and allow_extract and video_path and os.path.exists(video_path):
        ...
        try:
            log("No speech map yet - quickly extracting audio to find where speech occurs "
                "(keeps subtitle splits from spanning silences)...", "step")
            ...
            log(f"Could not extract audio to build the speech map: {e}", "warn")
            audio = None
    if not audio:
        log("No speech map: subtitle re-splitting will split proportionally by "
            "characters. For the best sync, delete the cached transcript to transcribe again.",
            "warn")
        return None
    ...
    speechmap.set_active(m)
    m.save(path)
    log(f"Built a speech map from the audio ({len(m)} marks) and saved it: "
...
def _asr_faster_whisper(audio_path: str, language: Optional[str], model_size: str,
    ...
    model = _MODEL_CACHE.get(key)
    if model is None:
        log(f"Loading faster-whisper '{model_size}' ({device}/{compute_type}) ...", "info")
...
def _dispatch(audio_path, backend, language, model_size, device, compute_type,
    ...
    raise ValueError(f"Invalid ASR backend: {backend!r} "
                     "(choose: paraformer | faster-whisper | sensevoice | whisperx)")
...
def _rescue_gaps(audio_path: str, segs: List[Segment], duration: float,
    ...
    if audio_gap_rescue:
        try:
            ...
            log(f"  Could not detect speech regions with ffmpeg: {e}", "warn")
            speech_ranges = []
        ...
        for rnd in range(1, max_rounds + 1):
            ...
            if audio_gap_rescue and speech_ranges:
                ...
                if speech_holes:
                    ...
                    log(f"  Audio scan: found {len(speech_holes)} non-silent regions "
                        f"with no transcript (total {speech_total/60:.1f} min, "
                        f"longest {longest:.1f}s).", "warn")

            gaps = merge_time_ranges(long_gaps + speech_holes,
                                     merge_gap=max(0.6, speech_gap_seconds * 0.5))
            if not gaps:
                msg = ("No significant gaps - the transcript covers the audio evenly."
                       if rnd == 1 else "No significant gaps remain after the rescue pass.")
                log(msg, "ok")
                break

            total_gap = sum(e - s for s, e in gaps)
            log(f"Rescue pass {rnd}: {len(gaps)} regions to re-check "
                f"(total {total_gap/60:.1f} min). Re-transcribing...", "step")
            ...
            for gi, (gs, ge) in enumerate(gaps, 1):
                ...
                try:
                    ...
                    log(f"  Could not cut the {gs:.0f}-{ge:.0f}s segment: {e}", "warn")
                    continue

                vol = _mean_volume_db(piece)
                if vol <= silence_db:
                    log(f"  [{gs/60:.1f}-{ge/60:.1f} min] silent ({vol:.0f} dB) - skipping.",
                        "info")
                    continue
                ...
                try:
                    ...
                    log(f"  Re-transcribing the segment at {gs/60:.1f} min failed: {e}", "warn")
                ...
                if sub_segs:
                    ...
                    added_total += added
                    log(f"  [{gs/60:.1f}-{ge/60:.1f} min] recovered {added} more lines.", "ok")
                try:
                    os.remove(piece)
                except OSError:
                    pass
            ...
            if added_total == 0:
                log("The rescue pass recovered no lines from the candidate regions - "
                    "it may be music/background noise, or the ASR already covered the speech.", "warn")
                break
    return segs
...
def transcribe(
    ...
    if initial_prompt:
        log(f"ASR vocabulary hint: {initial_prompt[:110]}"
    ...
    try:
        ...
        log(f"Backend '{backend}' failed on {device} ({e}).", "warn")
        primary_error = e
        recovered = False
        if not str(device or "").lower().startswith("cpu"):
            try:
                log("Retrying on CPU...", "info")
        ...
        if not recovered:
            if not fallback_backend or fallback_backend == backend:
                raise primary_error
            log(f"Switching to the fallback backend '{fallback_backend}' "
                f"because the primary backend still fails ({primary_error}).", "warn")
            try:
    ...
    log(f"Coverage: {rep['lines']} lines | speech {rep['covered_s']/60:.1f} min "
        f"of {rep['duration_s']/60:.1f} min ({rep['ratio']*100:.1f}%) | "
        f"last line at {rep['last_end_s']/60:.1f} min", "info")
    ...
    if rescue_gaps and duration > 0:
        ...
        segs = normalize_segments(segs, max_chars)
        rep = coverage_report(segs, duration)
        log(f"After rescue: {rep['lines']} lines | coverage {rep['ratio']*100:.1f}% | "
            f"last line at {rep['last_end_s']/60:.1f} min", "ok")

    # --- Lớp 4: bỏ câu bịa (Whisper "điền" quảng cáo kênh vào đoạn nhạc) ---
    if filter_hallucinations:
        segs, removed = drop_hallucinations(segs)
        if removed:
            log(f"Dropped {len(removed)} lines that look hallucinated (music/silence):",
                "warn")
            for line in removed[:5]:
                print(f"      - {line}")
            if len(removed) > 5:
                print(f"      ... and {len(removed) - 5} more lines")
            rep = coverage_report(segs, duration)
    ...
    if corrections:
        ...
        if nfix:
            log(f"Fixed {nfix} lines using the asr.corrections table.", "ok")

    # Cảnh báo to nếu vẫn thiếu nghiêm trọng
    if duration > 0 and rep["tail_ratio"] < 0.9:
        log(f"WARNING: the last line is only at {rep['last_end_s']/60:.1f} min while "
            f"the video is {rep['duration_s']/60:.1f} min long - the transcript MAY BE TRUNCATED. "
            "Check the transcript before dubbing.", "err")
    elif duration > 0 and rep["ratio"] < min_coverage:
        log(f"WARNING: only {rep['ratio']*100:.1f}% of the duration has speech. If the video "
            "has continuous speech this suggests MISSING PARTS - try a larger model "
            "(asr.model_size: medium) or another asr.backend.", "warn")

    sm = speechmap.get_active()
    if sm is not None and not sm.empty:
        log(f"Speech map: {len(sm)} character-level time marks - used to re-split "
            "subtitles exactly when the speaker talks (no interpolation across silences).", "ok")
    else:
        log("This backend returns no per-character timestamps - re-splitting "
            "will have to split proportionally, which drifts more easily.", "warn")

    log(f"Done: {len(segs)} transcript lines (backend={backend}, lang={lang}).", "ok")
    return segs, lang
