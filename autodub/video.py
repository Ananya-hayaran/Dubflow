...
def extract_audio(video: str, out_wav: str, sr: int = 16000,
    ...
    try:
        ...
        if m and float(m.group(1)) < -60:
            log("Mono mix is almost silent (the two channels may be out of phase) - "
                "trying the left channel only...", "warn")
            cmd2 = ["ffmpeg", "-y"]
            if trim_start > 0:
...
def ensure_audio(video: str, out_wav: str, sr: int = 16000,
    ...
    if target_duration:
        ...
        if preferred != out_wav:
            log(
                f"The ASR audio is long (~{_pcm_mib(target_duration, sr):.0f} MiB as WAV) "
                f"- saving {os.path.basename(preferred)} to save disk space.",
                "info",
            )
            out_wav = preferred
    if reuse_existing and os.path.exists(out_wav) and os.path.getsize(out_wav) > 1024:
        ...
        if audio_dur > 0 and (video_dur <= 0 or abs(audio_dur - video_dur) <= tolerance):
            log(f"Reusing extracted audio: {out_wav}", "ok")
            return out_wav
        log("Previously extracted audio does not match the video length - extracting again.", "warn")
...
def _mix_batch(items, total_ms, out_wav, sr, label="",
    ...
    """
    if label:
        log(f"  {label}: mixing {len(items)} clips...", "info")
    t0 = time.monotonic()
    ...
    if not prepared:
        ...
        if label:
            log(f"  {label}: finished in {time.monotonic() - t0:.1f}s.", "ok")
        return out_wav
    ...
    if len(prepared) == 1:
        ...
        # normalize=0 sums overlapping clips; without a limiter two loud voices
        # hard-clip when converted to 16-bit PCM. alimiter only acts on peaks
        # above 0.95 (quiet audio is untouched) and its 2 ms attack delays the
        # signal by 2 ms - measured, far below what is audible for dubbing.
        graph = ";".join(parts) + ";" + "".join(labels) + \
            f"amix=inputs={len(prepared)}:normalize=0:dropout_transition=0," \
            "alimiter=limit=0.95:attack=2:release=50:level=0[mixed]"
...
def _assemble_torch(
    ...
    for path, start_sec in items:
        try:
            ...
            log(f"  Skipping broken clip {os.path.basename(path)}: {e}", "warn")
            continue
...
def _assemble_ffmpeg_chunked(
    ...
    if active_end_ms > 0:
        ...
        if total_ms + chunk_ms < full_total_ms:
            log(
                f"The speech track has audio up to {total_ms / 1000 / 60:.1f} min; "
                "trailing silence is padded at render time to save disk space.",
                "info",
            )
    else:
    ...
    log(f"FFmpeg streaming mix: {n_chunks} chunks of at most {chunk_ms / 1000:.0f}s each.", "info")
    ...
    with tempfile.TemporaryDirectory(prefix="_mix_chunks_", dir=tmp_parent) as td:
        ...
        for ci in range(n_chunks):
            ...
            if n_chunks <= 20 or ci == 0 or ci + 1 == n_chunks or (ci + 1) % 10 == 0:
                log(f"  Chunk {ci + 1}/{n_chunks}: {len(active)} clips.", "info")

        log(f"Joining {len(chunk_files)} audio chunks into {os.path.basename(out_path)}...", "step")
...
def assemble_timeline_audio(
    ...
    if timeline_out != out_wav:
        log(
            f"The voice track is long (~{_timeline_pcm_gib(total_duration, sr):.1f} GiB as WAV) "
            f"- saving {os.path.basename(timeline_out)} to avoid the 4 GB WAV limit.",
            "info",
        )
    ...
    log(f"Mixing {len(items_sec)} clips onto the timeline...", "step")
    ...
    if skip_torch:
        log("Mixing audio with FFmpeg (avoids accelerated DLLs that Application Control may block).", "info")
    elif long_mix:
        try:
            log(f"Streaming mix (estimated RAM buffer {est_gib:.1f} GiB).", "info")
            _assemble_stream_torch(items_sec, total_duration, timeline_out, sr)
            log(f"Timeline mix (streaming) finished in {time.monotonic() - t_total:.1f}s.", "ok")
            return timeline_out
        except ImportError:
            log("torchaudio/soundfile are missing for the streaming mix - using the fallback.", "warn")
        except Exception as e:
            log(f"Streaming mix failed ({e}) - using the fallback.", "warn")

    # ── Đường nhanh: torchaudio ─────────────────────────────────────────────
    if not skip_torch:
        try:
            _assemble_torch(items_sec, total_duration, timeline_out, sr)
            log(f"Timeline mix (torchaudio) finished in {time.monotonic() - t_total:.1f}s.", "ok")
            return timeline_out
        except ImportError as e:
            log(f"torchaudio is not usable ({e}) - using the FFmpeg fallback.", "warn")
        except Exception as e:
            log(f"torchaudio failed ({e}) - trying the streaming mix before the FFmpeg fallback.", "warn")
            try:
                _assemble_stream_torch(items_sec, total_duration, timeline_out, sr)
                log(f"Timeline mix (streaming) finished in {time.monotonic() - t_total:.1f}s.", "ok")
                return timeline_out
            except Exception as e2:
                log(f"The streaming mix also failed ({e2}) - using the FFmpeg fallback.", "warn")
    ...
    log(f"FFmpeg mix: {n_batches} logical batch(es) of at most {batch} clips.", "info")

    if len(items_ms) <= batch and timeline_out == out_wav:
        _mix_batch(items_ms, total_ms, timeline_out, sr, label="Batch 1/1")
        log(f"Timeline mix (FFmpeg) finished in {time.monotonic() - t_total:.1f}s.", "ok")
        return timeline_out
...
def render_final(
    ...
    if src_dur <= 0:
        raise RuntimeError("Could not read the source video duration; stopping to avoid writing a broken file.")
    ...
    if force_h264 and not need_reencode and (src_codec not in ("h264", "") or ten_bit):
        need_reencode = True
        log(f"Source video is {src_codec.upper() or '?'}"
            + (f" {src_pix}" if ten_bit else "")
            + " - Windows Photos cannot open this type. Converting to H.264 "
              "so it plays everywhere (disable with force_h264: false).", "info")
    ...
    if delogo:
        ...
        try:
            ...
            log(f"Ignoring delogo (format must be 'x:y:w:h'): {delogo!r}", "warn")
        else:
            d = overlays.clamp_delogo({"x": x, "y": y, "w": w, "h": h}, vw, vh)
            if not d:
                log(f"Ignoring delogo {delogo!r}: it is outside the {vw}x{vh} frame "
                    "or the area is too small.", "warn")
            else:
                if (d["x"], d["y"], d["w"], d["h"]) != (x, y, w, h):
                    log(f"delogo {x}:{y}:{w}:{h} touches/exceeds the {vw}x{vh} frame edge - "
                        f"shrunk to {d['x']}:{d['y']}:{d['w']}:{d['h']} (ffmpeg needs "
                        "a border around the area).", "warn")
    ...
    if subtitle_srt:
        ...
        vlabel = "vs"
        log(f"Burning subtitles into the video ({style.split(',')[2]})", "info")
    ...
    log("Render " + ("(GPU NVENC)" if gpu and need_reencode
        ...
        + (", decoding on the GPU as well" if bac_hw[0] == "full" else "") + " ...",
        "step")
    ...
    def _run_render(use_nvenc: bool, hw: str = "none"):
                ...
                try:
                    ...
                    if pct >= last_pct + 5:
                        ...
                        if pct > 0:
                            ...
                            log(f"  Render: {pct}% | elapsed {elapsed:.0f}s | ~{eta:.0f}s left", "info")
                        else:
                            log(f"  Render: {pct}% | elapsed {elapsed:.0f}s", "info")
                        last_pct = pct
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                loi_ffmpeg = RuntimeError(
                    f"FFmpeg render failed ({result.returncode}):\n{err}")
        except InterruptedError:
            raise
        ...
        if loi_ffmpeg is not None:
            raise loi_ffmpeg
        log(f"  Render finished in {time.monotonic() - t_render:.1f}s.", "ok")
    ...
    for hw in bac_hw:
        try:
            ...
            log(f"GPU path '{hw}' did not work "
                f"({str(e).splitlines()[-1][:100]}) - retrying with a lower tier...",
                "warn")
    if loi_cuoi is not None:
        ...
        log(f"NVENC could not render ({str(loi_cuoi).splitlines()[0][:100]}) - "
            "switching to CPU x264...", "warn")
        _run_render(False)
    ...
    if not out_codec:
        raise RuntimeError("The output file has NO video stream - the render is broken.")
    elif src_dur > 0 and out_dur + max_drift < src_dur:
        raise RuntimeError(
            f"The output file is truncated: {out_dur:.1f}s but the source video is {src_dur:.1f}s.")
    elif src_dur > 0 and abs(out_dur - src_dur) > max_drift:
        log(f"The output file is {out_dur:.1f}s but the source video is {src_dur:.1f}s "
            "- it may be truncated.", "warn")
    elif out_codec == "h264" and out_pix == "yuv420p":
        log(f"Final video: H.264 8-bit, {out_dur:.1f}s - plays on Windows "
            "Photos / phones / the web.", "ok")
    else:
        log(f"Final video: {out_codec.upper()} {out_pix}, {out_dur:.1f}s. Note: "
            "Windows Photos may NOT open it - use VLC, or enable "
            "force_h264: true.", "warn")
    return out_path
